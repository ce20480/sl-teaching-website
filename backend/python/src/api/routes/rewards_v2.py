# api/routes/rewards_v2.py
from fastapi import Depends, APIRouter, Query, Body, BackgroundTasks, HTTPException
...
router = APIRouter(prefix="/rewards/v2", tags=["rewards_v2"])
...
async def award_xp(
    ...
    xp_service: XpRewardServiceV2 = Depends(get_xp_reward_service_v2),
    max_retries: int = 3
):
    ...
    try:
        ...
        # Validate the XP service is properly initialized
        if not hasattr(xp_service, 'contract') or xp_service.contract is None:
            logger.error("XP service contract not initialized properly")
            return {
                'status': 'error',
                'error': 'Contract not initialized properly. Check contract address and ABI.',
                'error_category': 'contract_initialization_error'
            }
...
async def mint_achievement(
    ...
    background_tasks: BackgroundTasks = None,
    achievement_service: AchievementRewardServiceV2 = Depends(get_achievement_reward_service_v2),
    max_retries: int = 3
):
