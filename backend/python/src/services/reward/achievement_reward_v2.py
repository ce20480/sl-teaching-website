...
class AchievementRewardServiceV2(BaseContractService):
    ...
    def __init__(self, blockchain: BlockchainService):
        ...
        try:
            contract = blockchain.contracts.get("AchievementToken")
            if not contract:
    ...
    def _get_achievement_token_abi(self) -> List[Dict[str, Any]]:
        """Return the ABI for AchievementToken with the functions we need"""
        try:
            # Try multiple possible locations for the contract file
            possible_paths = [
                # Root project directory
                os.path.join(os.path.dirname(os.path.dirname(os.path.dirname(os.path.dirname(os.path.dirname(__file__))))), 'contracts', 'AchievementToken.json'),
                # One level up from backend
                os.path.join(os.path.dirname(os.path.dirname(os.path.dirname(os.path.dirname(os.path.dirname(os.path.dirname(os.path.dirname(__file__))))))), 'contracts', 'AchievementToken.json'),
                # Inside backend directory
                os.path.join(os.path.dirname(os.path.dirname(os.path.dirname(os.path.dirname(os.path.dirname(__file__))))), 'contracts', 'AchievementToken.json'),
                # Current directory
                os.path.join(os.path.dirname(__file__), 'AchievementToken.json')
            ]
            ...
            if contract_json is None:
                raise FileNotFoundError("Could not find AchievementToken.json in any of the expected locations")
...
# Dependency provider for AchievementRewardServiceV2
def get_achievement_reward_service_v2(blockchain: BlockchainService = Depends(get_blockchain_service)):
    return AchievementRewardServiceV2(blockchain)
