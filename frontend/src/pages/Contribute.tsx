...
import { Upload, Wallet, Loader2, FileType, Brain, Trophy, Settings, CheckCircle, XCircle } from "lucide-react";
...
export default function Contribute() {
  ...
  const [handLandmarks, setHandLandmarks] = useState<number[] | undefined>(undefined);
  ...
  const [rewardPollingTaskId, setRewardPollingTaskId] = useState<string | null>(null);
  const [rewardPollingInterval, setRewardPollingInterval] = useState<NodeJS.Timeout | null>(null);

  // Handle file selection
  const handleFileSelect = (file: File) => {
    // Verify that the file is an image before proceeding
    if (!file.type.startsWith('image/')  && !file.type.endsWith('.png') && !file.type.endsWith('.jpg') && !file.type.endsWith('.jpeg')) {
  ...
  const handleUpload = async () => {
    ...
    try {
      ...
      if (!evaluationResult.success) {
        ...
        toast.error(evaluationResult.message || "Evaluation failed. Please try again.");
      ...
      const initialStatus: EvaluationStatus = {
        ...
        completed: false,
        phases: {
          evaluation: evaluationResult.evaluation || { status: "completed", success: true },
      ...
      if (!uploadResult.success) {
        ...
        if (uploadResult.duplicate || 
          ...
          const duplicateStatus: EvaluationStatus = {
            ...
            completed: true,
            phases: {
              evaluation: initialStatus.phases?.evaluation || { status: "completed", success: true },
              upload: { 
                status: "failed", 
                success: false, 
              ...
              reward: { status: "failed", success: false, error: "Duplicate files are not eligible for rewards" }
            }
          };
          ...
          toast.error("This file appears to be a duplicate. Please try a different image.");
          setIsUploading(false);
          return; // Exit early - don't process rewards for duplicates
        }
        
        // Handle other upload failures
        toast.error(uploadResult.message || "Failed to upload the file. Please try again.");
        setIsUploading(false);
        return;
      }
      ...
      if (uploadResult.success && 
        ...
        console.log("Upload was reported as successful but marked as duplicate");
      ...
      const uploadStatus: EvaluationStatus = {
        ...
        phases: {
          ...
          reward: initialStatus.phases?.reward || { status: "pending", success: false }
        }
      };
      ...
      if (!rewardResult.success) {
        ...
        if (rewardResult.duplicate || 
            ...
            (rewardResult.message && rewardResult.message.includes("duplicate")) ||
          ...
          toast.warning("Reward processing has been queued but may take some time to complete.");
        }
      } else {
        console.log("Reward processing started:", rewardResult);
        toast.success("Reward processing has started. You can track the status here.");
      }
  ...
  useEffect(() => {
    ...
    const checkStatus = async () => {
      ...
      if (rewardPollingTaskId === taskId) {
        console.log(`Skipping general status polling for task ${taskId} - using reward-specific polling`);
        setIsPolling(false);
        return;
      }
      ...
      try {
        ...
        if (status.status === "approved" && 
            ...
            (!status.phases?.reward?.success && !status.reward?.xp?.transaction_hash)) {
          console.log("Upload completed successfully, switching to reward-specific polling");
          setIsPolling(false);
        ...
        if (statusCheckErrors >= 3) {
          toast.error("Failed to check evaluation status. The background task is still processing.");
    ...
    const handleCompletionStatus = (status: EvaluationStatus) => {
      ...
      const isDuplicate = 
        ...
        (status.phases?.upload?.error && status.phases.upload.error.toLowerCase().includes("duplicate")) ||
        (status.message && status.message.toLowerCase().includes("duplicate"));
        
      if (isDuplicate) {
        toast.error("This file was already uploaded previously. Please try a different image.");
        return;
      }
      
      if (status.status === "approved") {
        if (status.phases?.reward?.error && status.phases.reward.error.includes("rate limit")) {
          toast.info("Contribution approved! XP rewards will be processed later due to blockchain rate limits.");
          ...
          toast.info("Contribution approved! XP rewards are being processed...");
        } else {
  ...
  return (
      ...
      <div>
              <h3 className="font-semibold text-amber-800">Duplicate File Detected</h3>
              <p className="text-amber-700 mt-1">
                This image has already been uploaded to our dataset. Please submit a different image.
              </p>
              <Button 
                variant="outline" 
          ...
          <div key={p.key} className="space-y-1">
            <div className="flex gap-2 items-center">
              <div className={cn(
                ...
                p.phase.success ? "bg-green-100 text-green-800" :
                p.phase.status === "failed" ? "bg-red-100 text-red-800" :
                p.phase.status === "completed" ? "bg-blue-100 text-blue-800" :
              ...
              <h4 className="font-medium flex items-center gap-1">
                ...
                {p.phase.status === "failed" || (p.phase.status === "completed" && !p.phase.success) ? (
            ...
            <div className={cn(
              "ml-8 text-sm",
              p.phase.success ? "text-green-600" :
              p.phase.status === "failed" || (p.phase.status === "completed" && !p.phase.success) ? "text-red-600" :
              "text-gray-500"
            )}>
              {p.phase.success ? (
                "Completed successfully"
              ) : p.phase.status === "failed" || (p.phase.status === "completed" && !p.phase.success) ? (
                p.key === "reward" && p.phase.error ? `Failed: ${p.phase.error}` : 
                p.key === "upload" && p.phase.details?.duplicate ? "Failed: Duplicate file detected" : 
                "Failed"
              ) : p.phase.status === "processing" ? (
                "Processing..."
              ) : (
            ...
            {p.key === "evaluation" && p.phase.details && (
              <div className="mt-1 text-xs text-gray-500">
                {p.phase.details.blur_score !== undefined && 
                  <div>Image quality: {(Number(p.phase.details.blur_score) * 100).toFixed(0)}%</div>}
                {p.phase.details.landmark_score !== undefined && 
                  <div>Hand detection: {(Number(p.phase.details.landmark_score) * 100).toFixed(0)}%</div>}
            ...
            {p.key === "upload" && p.phase.success && p.phase.details && (
              <div className="mt-1 text-xs text-gray-500">
                {p.phase.details.duplicate && (
                  <div className="text-amber-600 font-medium">File already exists in storage</div>
                )}
                {p.phase.details.cid && typeof p.phase.details.cid === 'string' && (
                  <div className="truncate">
                    CID: <span className="font-mono">{p.phase.details.cid.substring(0, 10)}...</span>
                  </div>
                )}
              </div>
            )}
            ...
            {p.key === "reward" && p.phase.success && p.phase.details?.xp && (
              <div className="mt-1 text-xs text-green-600 font-medium">
                Earned: +{typeof p.phase.details.xp === 'object' ? 
                  (p.phase.details.xp as {amount?: number}).amount || 0 : 0} XP
              </div>
            )}
  ...
  const startRewardStatusPolling = (taskId: string) => {
    ...
    const checkRewardStatus = async () => {
      try {
        pollCount++;
        console.log(`Checking reward status for task ${taskId} (poll #${pollCount})`);
        const status = await storageApi.checkContributionStatus(taskId);
        ...
        console.log(`Reward phase status check: status=${rewardStatus}, success=${rewardSuccess}, hasHash=${!!hasTransactionHash}`);
        ...
        if (
          ...
          console.log(`Reward processing completed for task ${taskId} with status: ${rewardSuccess ? 'success' : 'failed'}`);
          
          // Display appropriate message
          if (rewardSuccess || hasTransactionHash) {
            toast.success("Reward transaction completed successfully! XP has been awarded.");
        ...
        if (rewardPollingInterval && (pollCount === 3 || pollCount === 8)) {
          ...
          console.log(`Increasing polling interval to ${newInterval / 1000} seconds after ${pollCount} polls`);
        ...
        console.error(`Error checking reward status for task ${taskId}:`, error);
        
        // After 5 consecutive errors, stop polling to avoid infinite errors
        setStatusCheckErrors(prev => {
          if (prev >= 5) {
            console.log("Too many errors checking reward status, stopping poll");
            if (rewardPollingInterval) {
              clearInterval(rewardPollingInterval);
              setRewardPollingInterval(null);
            }
        ...
        <CardHeader>
          <div className="flex items-center">
            ...
            <div>
              ...
              <CardDescription>
                Upload images or videos of sign language gestures to help train our model
              </CardDescription>
            </div>
          </div>
        </CardHeader>
            ...
            <div
              className={`mb-6 p-4 rounded-lg ${
                evaluationStatus.status === "approved"
                  ...
                  : evaluationStatus.status === "rejected" && evaluationStatus.phases?.upload?.details?.duplicate
            ...
            >
              ...
              <p className="mt-2 text-sm">
                {evaluationStatus.status === "rejected" && evaluationStatus.phases?.upload?.details?.duplicate
                  ...
                  : evaluationStatus.message || "Your contribution is being processed."}
              </p>
              {renderPhasedStatus()}
            </div>
          )}
          ...
          {!isConnected ? (
            <div className="text-center py-8">
              ...
              <p className="text-gray-600 mb-4">
                You'll need to connect your wallet to earn rewards for contributing data.
              </p>
              <WalletConnect />
            </div>
              ...
              {contributions.map(contribution => (
                <div
                  ...
                  className={`p-3 rounded-lg border cursor-pointer transition-all 
                    ${contribution.taskId === selectedContribution ? 'bg-blue-50 border-blue-300' : 'bg-white border-gray-200 hover:border-blue-200'}`}
                  onClick={() => handleContributionSelect(contribution.taskId)}
                >
                  <div className="flex justify-between items-center">
                    <div className="flex items-center space-x-2">
                      <FileType className="h-4 w-4 text-blue-600" />
                      <span className="font-medium truncate max-w-[180px]">{contribution.fileName}</span>
                    </div>
                    <div className="flex items-center">
                      <span className={`text-xs px-2 py-1 rounded-full ${getStatusBadgeColor(contribution.status)}`}>
                        {contribution.status}
                      </span>
                    </div>
                  </div>
